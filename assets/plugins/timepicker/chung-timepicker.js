	...
	$.chungTimePicker = function(element, options) {
		...
		function createHoursWrap() {
			var table = $('<table class="table-condensed"></table>'),
				title = $('<thead><tr><th colspan="6" class="title">Horas</th></tr></thead>'),
				hours = $('<tbody></tbody>'),
		...
		function createMinutesWrap() {
			...
			switch(obj.options.viewType) {
				case 0:
					title.find('tr').append('<th class="prev js-back-hours"><i class="icon-arrow-left"></i></th><th colspan="4" class="title">Minutos</th>');
					break;
				case 1:
					title.find('tr').append('<th colspan="6" class="title">Minutos</th>');
					break;
				default:
					console.log(obj.errorPrefix + 'viewType Error');
					break;
		...
		function createBottomOperate() {
			switch(obj.options.viewType) {
				case 0:
					...
					break;
				case 1:
					obj.timepicker_bottom.empty().append('<span class="bottom-btn js-cancel">Cancelar</span>')
						.append('<span class="bottom-btn js-confirm">Confirmar</span>');
